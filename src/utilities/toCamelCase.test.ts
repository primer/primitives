...
describe('Utilities: toCamelCase', () => {
  ...
  it('it transforms all lowercase sentence (words with spaces)', () => {
    expect(toCamelCase('primer design token')).toStrictEqual('primerDesignToken')
  })

  it('it transforms all words with special chars', () => {
    expect(toCamelCase('primer_design-token+edition')).toStrictEqual('primerDesignTokenEdition')
  })
