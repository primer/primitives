...
import {colorToHexAlpha} from './colorToHexAlpha.js'
...
describe('Transformer: colorToHexAlpha', () => {
  it('transforms hex3, hex6 `color` tokens with alpha value', () => {
    ...
    expect(input.map(item => colorToHexAlpha.transform(item, {}, {}))).toStrictEqual(expectedOutput)
  })
  ...
  it('transforms hex8 `color` tokens with alpha value, ignoring the initial alpha from the hex8', () => {
    ...
    expect(colorToHexAlpha.transform(input, {}, {})).toStrictEqual('#34343499')
  })

  it('transforms rgb `color` tokens with alpha value', () => {
    const input = getMockToken({value: 'rgb(100,200,255)', alpha: 0.6})
    expect(colorToHexAlpha.transform(input, {}, {})).toStrictEqual('#64c8ff99')
  })
  ...
  it('transforms rgba `color` tokens with alpha null, ignoring alpha', () => {
    ...
    expect(colorToHexAlpha.transform(input, {}, {})).toStrictEqual('#64c8ff80')
  })
})
