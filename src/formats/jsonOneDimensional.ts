...
import {prefixAllTokensArray} from './utilities/prefixAllTokensArray'
 ...
 */
export const jsonOneDimensional: StyleDictionary.Formatter = ({dictionary, file: _file, options, platform}) => {
  // add prefix if defined
  const allTokens = prefixAllTokensArray(dictionary.allTokens, platform)
  // option to allow verbose output (object) or just the value
  const {outputVerbose} = options
  const tokens = jsonToFlat(allTokens, outputVerbose)
