...
In these themes, green, red and orange are changed to different colors. This allows us to provide the right color contrast in the UI for people with color vision deficiency (cvd). For example, when comparing branches in pull requests.

To make this easy to maintain, scales are adjusted instead of changing the references of functional tokens. This results in a mismatch of color scale names and the value they hold. For example `scale.green` is actually blue in cvd modes. This is not a bug but desired for easier maintenance. We discourage designing with any of the cvd modes, so this should not create any confusion for our users.
