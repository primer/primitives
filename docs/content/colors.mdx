...
When changing themes, we adjust the scales instead of changing which scale a functional tokens references. This results in a mismatch of color scale names and the value they hold. For example, the green scale uses blue in cvd modes. This is not a bug but is desired for easier maintenance. Color scales should not be used directly, so this mismatch is an internal implementation detail not visible to out users.
